...
async def search(input_: Input):
    ...
    for index in indexes:
        ...
        for _score, _cand in zip(scores, candidates):
            candidates.append({
                'raw_score': _score,
                'candidate': _cand,
                'indexer': indexer['name'],
                'score': _score # is hnsw compute dot-prod # entity encs are required
            })
    return candidates


def load_models(args):
    assert args.indexes is not None, 'Error! Index is required.'
...
if __name__ == '__main__':
    ...
    # indexer
    parser.add_argument(
        "--indexes", type=str, default=None, help="comma separate list of paths to load indexes [type:path:name:ro/rw] (e.g: hnsw:index.pkl:wiki:ro,flat:index2.pkl:local:rw)",
    )
    parser.add_argument(
