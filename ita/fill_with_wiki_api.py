...
def wiki_href2id(href, title2id):
    ids = []
    
    href_string = '|'.join([h for h in href if h is not None])
    href_string = quote(href_string)
    url = 'https://it.wikipedia.org/w/api.php?action=query&titles={}&format=json&redirects'.format(href_string)
