...
tasks = [
    # {
    #     'name': 'aida_under_bi10_max_stdev4_hamming',
    ...
    # },
    # {
    #     'name': 'aida_under_bi10_max_stdev4',
    #     'train': ['dataset_and_preds/AIDA-YAGO2_train.csv'],
    #     'test': ['dataset_and_preds/AIDA-YAGO2_testa.csv', 'dataset_and_preds/AIDA-YAGO2_testb.csv'],
    #     'sampling': 'undersample',
    #     'features':  [
  ...
  'train': 'train_new_out_ablation_1001/aida_under_bi_max_stats10_levenshtein_jaccard_model.pickle'},
 {'features': ['cross_stats_10_max',
               'cross_stats_4_stdev',
               'bi_stats_10_max',
               'bi_stats_4_stdev'],
...
for task in tasks:
    ...
    if isinstance(task['train'], str):
        ...
        train_value_counts = pd.DataFrame(train_df['y_bi'].value_counts()).to_markdown()

        X_train = train_df[task['features']].values
        y_train = train_df['y_bi'].values
...
if only is not None and os.path.isfile(os.path.join(outpath, 'train_new_summary.csv')):
