...
```{python}
print('Accuracy:', (th_df['y'] == th_df['y_pred_lr']).sum() / th_df.shape[0])
```

```{python}
print('Recall:', th_df.query('y==1 and y_pred_lr ==1').count()[0] / th_df.query('y==1').shape[0])
```

```{python}
print('Precision:', th_df.query('y==1 and y_pred_lr ==1').count()[0] / th_df.query('y_pred_lr==1').shape[0])
```

```{python}
print('FPR:', th_df.query('y==0 and y_pred_lr ==1').count()[0] / th_df.query('y_pred_lr==1').shape[0])
```

```{python}
print('F Precision:', th_df.query('y==0 and y_pred_lr ==0').count()[0] / th_df.query('y_pred_lr==0').shape[0])
```

```{python}
print('F Recall:', th_df.query('y==0 and y_pred_lr ==0').count()[0] / th_df.query('y==0').shape[0])
```

```{python}
print('FNR:', th_df.query('y==1 and y_pred_lr ==0').count()[0] / th_df.query('y_pred_lr==0').shape[0])
```

```{python}
print('FN:', th_df.query('y==1 and y_pred_lr ==0').count()[0] / th_df.query('y==1').shape[0])
```
...
# TODO
- [partial] try to fix a threshold for the NIL classification to maximize performance
- [partial] LinearRegression as baseline
- how to evaluate various methods/metrics
