...
```{python}
output_path = os.path.join('..', 'output_scores_indexer+nil_20200826')
```
...
```{python}
_all = []
for l in cross_df['unsorted_scores'].values:
    _all.extend(l)
```

```{python}
len(cross_df.iloc[0]['unsorted_scores'])
```

```{python}
_sall = list(map(sigmoid, _all))
```

```{python}
sdf = pd.DataFrame(_sall)
```

```{python}
sdf.plot(kind='density')
```
...
```{python}
cross_stats['correct'].dropna().apply(sigmoid).plot(kind='density')
```
...
for d in ['train', 'test', 'train_hard', 'test_hard', 'train_aug', 'train_hard_aug']:
    datasets[d] = pd.read_csv('dataset/train')
```
...
```{python}
res.sort_values('FNR', ascending=True)
```
