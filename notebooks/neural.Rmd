...
```{python}
print(classification_report(y_test, torch.round(y_pred).detach().numpy().reshape(-1,).astype(int)))
```

```{python}
# roc curve
fpr, tpr, thresholds = sklearn.metrics.roc_curve(y_test, y_pred.detach().numpy().reshape(-1,))
pd.DataFrame({
    'tpr': tpr,
    'fpr': fpr,
